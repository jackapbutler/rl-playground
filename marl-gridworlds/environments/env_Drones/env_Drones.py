        ...
        for i in range(obs_size):
            for j in range(obs_size):
                ...
                for k in range(self.human_num):
                    if (
                        self.human_list[k].pos[0] == x
                        and self.human_list[k].pos[1] == y
                    ):
                        obs[i, j, 0] = 1
                        obs[i, j, 1] = 0
                        obs[i, j, 2] = 0
                ...
                if (drone.view_range - 1 - i) * (drone.view_range - 1 - i) + (
                    drone.view_range - 1 - j
        ...
        for k in range(self.drone_num):
            ...
            for i in range(size):
                for j in range(size):
                    x = (
                        i
                        + self.drone_list[k].pos[0]
                        - self.drone_list[k].view_range
                        + 1
                    )
                    y = (
                        j
                        + self.drone_list[k].pos[1]
                        - self.drone_list[k].view_range
                        + 1
                    )
                    if_obs = True
        ...
        for k in range(self.human_num):
            if human_act_list[k] == 0:
                if self.human_list[k].pos[0] > 0:
                    free_space = self.land_mark_map[
                        self.human_list[k].pos[0] - 1, self.human_list[k].pos[1]
                    ]
                    if free_space == 0:
                ...
                if self.human_list[k].pos[1] > 0:
                    free_space = self.land_mark_map[
                        self.human_list[k].pos[0], self.human_list[k].pos[1] - 1
                    ]
                    if free_space == 0:
